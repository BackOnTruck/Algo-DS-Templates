...
Int a[300001],b[300001],c[300001],d[300001],f[300001],G[300001];
inline void mul(int p,int q) // input: a,b, output: a
{
	int n=1,k=-1;
	while(n<=p+q)n<<=1,k++;
...
void inverse(int n) //input: c, output: b
{
	if(n==1)
	{
		b[0]=inv(c[0].r);
...
void Sqrt(int n) // input: c, output: d
{
	logarithm(n);
	for(int i=0;i<n;i++)f[i]=a[i]*499122177;
	exponent(n);
}
int main()
{
	...
	memset(a,0,sizeof a);
	memset(b,0,sizeof b);
	for(int i=0;i<n;i++)a[i]=d[i],b[i]=G[i];
	mul(n,n);
	printf("0");
	for(int i=0;i<n-1;i++)printf(" %d",(a[i]*inv(i+1)).r);
	puts("");
}
