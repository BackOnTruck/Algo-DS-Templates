...
Int a[300001],b[300001],c[300001],d[300001],f[300001],G[300001],H[300001];
...
int main()
{
	...
	const int I=86583718,inv2=inv(2),inv2i=inv(2*I);
	if(!op)
	{
		...
		memset(d,0,sizeof d);
		for(int i=0;i<n;i++)f[i]=G[i]*(mod-I);
		exponent(n);
		for(int i=0;i<n;i++)H[i]-=d[i];
		for(int i=0;i<n;i++)H[i]*=inv2i;
	}
	else
	{
		for(int i=0;i<n;i++)f[i]=G[i]*I;
		...
		exponent(n);
		for(int i=0;i<n;i++)H[i]+=d[i];
		for(int i=0;i<n;i++)H[i]*=inv2;
	}
	for(int i=0;i<n;i++)printf("%d ",H[i].r);
	puts("");
}
